 ...
 *
 * F_lateral is the total centripetal force the tyre must supply (N).
 * For a single-track vehicle the full force acts at one contact patch —
 * there is no lateral load split between tracks.
 *
 * PREVIOUS (WRONG) formula used m × a × Y_cg / trackWidth, which computes
 * a car-style lateral load transfer between two tracks. For a motorcycle
 * with trackWidth ≈ 0 this was physically incorrect and produced wildly
 * inflated numbers. trackWidth input is retained for future 3-D roll model.
 *
 * @param mass         Total mass m (kg)
...
export function computeCornering(
  ...
  cornerSpeed: number,
  cornerRadius: number,
  _Y_cg: number,         // retained for future 3-D use
  _trackWidth: number,   // retained for future 3-D roll model
