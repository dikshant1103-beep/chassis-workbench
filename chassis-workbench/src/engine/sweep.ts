...
const CHAIN_PITCH_SWEEP = 15.875; // mm — 520 chain pitch

/**
 * Compute anti-squat % for a given swingarm angle using the Foale
 * graphical / instant-centre method.
 *
 * Matches computeAntiSquatUnified (antiSquat.ts) exactly:
 *   - Chain force angle is AUTO-COMPUTED from sprocket geometry via
 *     computeChainForceAngle() — the user-supplied chain.chainForceAngle is ignored.
 *   - The IC line-2 is anchored at the UPPER-RUN TANGENT CONTACT POINT on the
 *     drive sprocket (not the sprocket center), giving consistent AS% values
 *     across the sweep chart and the Anti-Squat tab.
 *
 * @param theta_sa_rad  Swingarm angle at this travel step (rad)
 * @param X_sp          Swingarm pivot X from front axle (mm)
 * @param H_sp          Swingarm pivot height (mm)
 * @param wheelbase     WB at this travel step (mm, = X of rear axle)
 * @param H_ra          Rear axle height at this travel step (mm)
 * @param Y_cg          CoG height (mm)
 * @param chain         ChainParams (sprocket teeth + CS position)
 */
function antiSquatAtAngle(
  ...
  if (chain.isCVT || Math.abs(Y_cg) < 1e-9) return NaN;

  const r_drive = (chain.frontSprocket * CHAIN_PITCH_SWEEP) / (2 * Math.PI);
  const r_rear  = (chain.rearSprocket  * CHAIN_PITCH_SWEEP) / (2 * Math.PI);

  // Absolute countershaft position (frame-fixed, in old coords)
...
export function computeSweep(
  ...
  for (let i = 0; i < nSteps; i++) {
    ...
    // Motion ratio — MR = shock_compression / wheel_travel  (Foale convention, < 1 for monoshock)
    // When motionRatioOverride is provided, use it directly (bypass unreliable shock geometry).
    // Geometry path: MR = |dL/du| = |d(shock_length)/d(wheel_travel)|
    //   dL/du < 0 (shock shortens as wheel rises) → MR = |dL/du| = shock/wheel < 1 typically
    let MR: number;
    if (sweep.motionRatioOverride && sweep.motionRatioOverride > 0) {
      MR = sweep.motionRatioOverride;
    } else if (u < du_mm || u > (nSteps - 2) * du_mm) {
      const du2 = du_mm;
      const L1 = shockLenFn(u);
      const L2 = shockLenFn(u + (u < du_mm ? du2 : -du2));
      const sign = u < du_mm ? 1 : -1;
      const dL = sign * (L2 - L1) / du2;
      MR = Math.abs(dL);  // shock/wheel
    } else {
      // Central difference: |dL/du| = shock_compression per mm wheel travel
      const L_lo = shockLenFn(u - du_mm);
      const L_hi = shockLenFn(u + du_mm);
      MR = Math.abs((L_hi - L_lo) / (2 * du_mm));  // shock/wheel
    }

    const WR = k * MR * MR;  // WR = k × MR² (Foale Eq 7.1)

    // H_ra at this travel step: rear axle rises by u from static height
    const H_ra_u = gp.rearAxleHeight + u;
    const AS = antiSquatAtAngle(
      theta_sa, gp.swingarmPivotX, gp.swingarmPivotHeight,
      gp.wheelbase, H_ra_u, Y_cg_mm, chain,
    );
