 ...
 *
 * Front and rear sprung masses are then allocated proportional to the
 * static axle loads (same ratio as CoG position):
 *
 * m_sprung_front = m_sprung × (WB − X_cg) / WB               ... Eq 7.6
 * m_sprung_rear  = m_sprung × X_cg / WB                      ... Eq 7.7
 *
...
export function computeSprungMasses(
  ...
  // Eq 7.6 — front sprung mass proportional to front axle load share
  const sprungMassFront = sprungMass * (wheelbase - X_cg) / wheelbase;

  // Eq 7.7 — rear sprung mass
  const sprungMassRear = sprungMass * X_cg / wheelbase;
...
export function computeSuspension(
  ...
  // Normalised damping ratio — Eq 7.14
